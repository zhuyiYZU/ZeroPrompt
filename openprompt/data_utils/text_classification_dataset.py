    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf-8') as f:
            ...
            for idx, row in enumerate(reader):
                ...
                examples.append(example)
        return examples
class OhsumedProcessor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["Bacterial_Infections_and_Mycoses","Virus_Diseases","Parasitic_Diseases","Neoplasms","Musculoskeletal_Diseases","Digestive_System_Diseases","Stomatognathic_Diseases","Respiratory_Tract_Diseases",
                       "Otorhinolaryngologic_Diseases","Nervous_System_Diseases","Eye_Diseases","Urologic_and_Male_Genital_Diseases","Female_Genital_Diseases_and_Pregnancy_Complications",
                       "Cardiovascular_Diseases","Hemic_and_Lymphatic_Diseases","Neonatal_Diseases_and_Abnormalities","Skin_and_Connective_Tissue_Diseases", "Nutritional_and_Metabolic_Diseases","Endocrine_Diseases","Immunologic_Diseases","Disorders_of_Environmental_Origin","Animal_Diseases","Pathological_Conditions_Signs_and_Symptoms"]
...
class ClickbaitProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        path = os.path.join(data_dir, "{}.csv".format(split))
        examples = []
        csv.field_size_limit(2147483647)  # 设置为一个较大的值，例如2GB
        with open(path, encoding='utf8') as f:
            reader = csv.reader(f, delimiter=',')
            for idx, row in enumerate(reader):
                label, title,article = row[0],row[1],row[2]
                title = title.replace('\\', ' ')
                article = article.replace('\\', ' ')
                # summary = summary.replace('\\', ' ')
                if (label == "news" ):label = 0
                elif(label=="clickbait"):label = 1
                else :continue
                label = int(label)
                example = InputExample(guid=str(idx), text_a=title,label=int(label))#text_b=article ,
                examples.append(example)
        return examples
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(path, encoding='utf8') as f:
            ...
            for idx, row in enumerate(reader):
                ...
                examples.append(example)
        return examples
class TwtProcessor(DataProcessor):
...
class IHSProcessor(DataProcessor):

    def __init__(self):
        super().__init__()
        self.labels = ["friendly","offensive"]
...
class COLDProcessor(DataProcessor):
...
PROCESSORS = {
    ...
    "snippets": SnippetsProcessor,
    "tweet": TwtProcessor,
    'ohsumed': OhsumedProcessor,
    "implict_hatespeech":IHSProcessor,
    "COLD_chinese":COLDProcessor,
    "chinese_implict":CNIProcessor,
    "chatgpt_error":CHATProcessor
}
