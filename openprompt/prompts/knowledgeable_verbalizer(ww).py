...
class KnowledgeableVerbalizer(ManualVerbalizer):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__(classes=classes, prefix=prefix, multi_token_handler=multi_token_handler, tokenizer=tokenizer,
                         **kwargs)
    ...
    def generate_parameters(self) -> List:
        ...
        print("##Num of label words for each label: {}".format(self.label_words_mask.sum(-1).cpu().tolist()),
              flush=True)
    ...
    def register_calibrate_logits(self, logits: torch.Tensor):
        ...
        rm_calibrate_ids = set(
            torch.argsort(self._calibrate_logits)[:int(self.candidate_frac * logits.shape[-1])].cpu().tolist())
    ...
    def aggregate(self, label_words_logits: torch.Tensor) -> torch.Tensor:
        ...
        if not self.training:
            label_words_weights = F.softmax(
                self.pred_temp * self.label_words_weights - 10000 * (1 - self.label_words_mask), dim=-1)
        else:
	...
	#x=label_words_weights.item()
        print(label_words_weights.float())
        return label_words_logits
